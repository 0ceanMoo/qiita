...
tags:
  ...
  - Husky
private: true
updated_at: '2026-08-05T12:54:57+09:00'
...
どちらも「思い込みで決めない」を意識した。特にCIの配置は、感覚で「軽いはず」と思っていたチェックが実測で全然軽くなかった、という話が出てくる。
...
だから「どのチェックを、どのタイミングに置くか」を最適化する。置き場所は思い込みで決めず、実測して検証する。これがこの章の内容。
...
**1ファイルに絞っても17.8秒、209ファイル全体でも19.8秒。** ファイル数が209倍になっても大差ない（検査対象の数より、数百本のルールを読み込んでパース・検証する固定コストの方が支配的なため）。
...
**教訓**: チェックの配置は「差分だから軽いはず」という思い込みではなく、実測で決める。

#### `feature/**` へのpushでCIを実行しない

ローカルフックが同等のチェックを担い、develop宛PRが最終ゲートになるため。以前はpushとPRの両方でワークフローが起動し、同一コミットに対してジョブが2回走っていた。
...
`concurrency` は、PRへの再pushでは古い実行をキャンセルするが、**`main` へのpushではキャンセルしない**。キャンセルするとそのコミットが緑だった証跡が残らないため。
...
厳密には、同じ `group` 内でpendingになれるのは1件だけなので、`main` に短時間で3回以上pushすると、中間のコミットの実行はキャンセルされて証跡が残らないことがある。それでも直近のpushの検査結果は必ず残る。
...
- **強制できない制約は、検知で補う。** ブランチ保護ルールが働いていないなら、`push: main` でCIを回し、経路を問わず壊れた変更を事後検知する
